...
{
	public const float ScriptVersion = 2.98f;
	...
	public AVProMovieCaptureMouseCursor _mouseCursor;
	...
	public virtual bool PrepareCapture()
	{
		...
		else
		{
			if (_useMotionBlur && _motionBlurSamples > 1)
			{
				...
				if (this is AVProMovieCaptureFromTexture || this is AVProMovieCaptureFromCamera360)
				{
					if (_motionBlur == null)
					{
						_motionBlur = this.GetComponent<AVProMovieCaptureMotionBlur>();
					}
					if (_motionBlur == null)
					{
						_motionBlur = this.gameObject.AddComponent<AVProMovieCaptureMotionBlur>();
					}
